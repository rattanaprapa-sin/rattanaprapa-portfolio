...
export default function ProjectModal({ project, onClose }) {
  ...
  return (
    <>
      <div
        className="modal-backdrop fade show blur-backdrop"
        onClick={onClose}
        style={{ zIndex: 1040 }}
      ></div>
      <div
        ...
        >
          <div className="modal-content rounded-4 border-0 shadow-lg overflow-hidden">
            <div className="modal-header border-0 pb-0 position-absolute top-0 end-0 z-3 m-2">
              <button
                type="button"
                className="btn-close bg-body rounded-circle p-2 shadow-sm"
                aria-label="Close"
                onClick={onClose}
                style={{ opacity: 0.9 }}
              ></button>
              ...
              <div className="row g-0 project-modal-row">
                ...
                <div className="col-lg-7 bg-dark position-relative d-flex align-items-center overflow-hidden">
                  {project.modalImages && project.modalImages.length > 1 ? (
                    <div
                      id={`projectImageCarousel-${project.title.replace(/\s+/g, '-')}`}
                      ...
                      <div className="carousel-indicators">
                        {project.modalImages.map((_, idx) => (
                          <button
                            key={idx}
                            type="button"
                            data-bs-target={`#projectImageCarousel-${project.title.replace(/\s+/g, '-')}`}
                ...
                <div className="col-lg-5 p-4 px-md-5 py-md-4 d-flex flex-column justify-content-center overflow-y-auto">
                  <div className="mb-3">
                    {project.tags.map((tag, i) => (
                      <span
                        key={i}
                        className="badge bg-primary bg-opacity-10 text-primary me-2 mb-2 px-3 py-2 rounded-pill"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  {project.date && (
                    <p className="text-body-secondary mb-4 opacity-75">{project.date}</p>
                  )}

                  <div className="mb-4">
                    <h5 className="fw-bold mb-3">About the Project</h5>
                    <p className="text-body-secondary lh-base" style={{ whiteSpace: 'pre-line' }}>
                  ...
                  {project.challenges && project.challenges.length > 0 && (
                    <div>
                      ...
                      <ul className="text-body-secondary lh-base mb-0 ps-3">
